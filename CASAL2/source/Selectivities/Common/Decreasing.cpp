 ...
 * Default constructor
 */
Decreasing::Decreasing(shared_ptr<Model> model) : Selectivity(model) {
  parameters_.Bind<Double>(PARAM_DECREASING_PARAMETER, &decreasing_, "Decreasing parameter", "", 0.0);
  ...
  RegisterAsAddressable(PARAM_DECREASING_PARAMETER, &decreasing_);
 ...
 * The core function
 */
Double Decreasing::get_value(Double value) {
  LOG_CODE_ERROR() << "Decreasing::get_value(Double value)";
  /*
  if (model_->partition_type() == PartitionType::kAge) {
  } 
  if (value < low_) {
 ...
 * The core function
 */
Double Decreasing::get_value(unsigned value) {
  if (value < low_) {
    return  0.0;
  } else if (value > high_) {
    return *v_.rbegin();
